...
import { View, Text, TextInput, Image, StyleSheet, ActivityIndicator, FlatList, Alert, Platform } from 'react-native';
...
const VendorsScreen = props => {
    ...
    useEffect(() => {
        const getData = async () => {
            setIsLoading(true);
            try {
                await dispatch(userAction.getData(0, UserType.Vendor));
                setIsLoading(false);
            } catch (err) {
                setIsLoading(false);
    ...
    const editClickHandler = id => {
        ...
        if (data.length > 0) {
            props.navigation.navigate('VendorEntry', { 'id': id });
        }
    }
    ...
    const documentClickHandler = id => {
        let data = users.filter((x) => x.id == id);
        if (data.length > 0) {
            props.navigation.navigate('Documents', { 'userId': id });
        }
    }
	...
	const viewClickHandler = (id) => {
		let data = users.filter((x) => x.id == id);
        if (data.length > 0) {
            props.navigation.navigate('VendorDetails', { 'id': id });
        }
	}
            ...
            <FlatList
                ...
                renderItem={({ item, index }) => (
                    <CardData
                        ...
                        hasActions={true}
                        actions={[viewClickHandler, counterClickHandler, editClickHandler, deleteClickHandler]}
                        actionIcons={[ActionIcons.View, ActionIcons.Person, ActionIcons.Edit, ActionIcons.Delete]}
                        actionIconColors={[ActionIconColors.View, ActionIconColors.Person, ActionIconColors.Edit, ActionIconColors.Delete]}
                    />
                )}
            />
        </View>
